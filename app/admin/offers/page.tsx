...
/* Simple inline-styled toggle — no CSS variables needed */
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className="relative shrink-0 h-6 w-11 rounded-full transition-colors duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2"
      style={{ backgroundColor: checked ? "#dc2626" : "#d1d5db" }}
    >
      <span
...
const labelCls = "text-[11px] font-extrabold text-zinc-400 uppercase tracking-widest block mb-1.5";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button
          onClick={openAdd}
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 font-bold px-5 py-3 rounded-2xl hover:opacity-90 active:scale-[0.98] transition-all text-sm text-white shadow-md"
      ...
      {/* ── Modal ──
          DialogContent default: fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[calc(100%-2rem)] sm:max-w-sm
          We override to: bottom-sheet on mobile, centered max-w-lg on desktop
      */}
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent
          showCloseButton={false}
          className="
            p-0 gap-0 flex flex-col overflow-hidden bg-white ring-0 shadow-2xl
            max-h-[92vh]
            ...
            sm:max-w-lg sm:w-full sm:rounded-3xl
          "
        >
          ...
          {/* Dark header — always visible (shrink-0 never compresses) */}
          <div className="relative bg-gradient-to-r from-zinc-900 to-zinc-800 sm:rounded-t-3xl px-5 py-4 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-400 mt-0.5">Offer will appear on every product page</p>
              </div>
            </div>
            <button
          ...
          <div className="overflow-y-auto px-5 py-5 space-y-4 flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select value={form.bankName} onChange={(e) => setForm({ ...form, bankName: e.target.value })} className={selectCls}>
                  {BANKS.map((b) => <option key={b}>{b}</option>)}
                </select>
              </div>
              <div>
                <label className={labelCls}>Card Type</label>
                <select value={form.cardType} onChange={(e) => setForm({ ...form, cardType: e.target.value })} className={selectCls}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                <label className={labelCls}>{form.discountType === "flat" ? "Amount (₹)" : "Percentage (%)"}</label>
            ...
            {/* Toggle row */}
            <div className="flex items-center justify-between bg-zinc-50 border border-zinc-100 rounded-2xl px-4 py-3.5 gap-4">
